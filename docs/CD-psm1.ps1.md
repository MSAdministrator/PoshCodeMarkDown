...
very simple module that’s add ‘cd -’ functionality. it basically keeps track of folders you have visited (using prompt function) and make those available in set-prevlocation function. alias cd- make it (almost) feel like original. you can add argument [int] that will limit list’s length. if you have other modules that modify prompt
